import numpy as np
import scipy.linalg as la
from qiskit import QuantumCircuit, transpile
from qiskit.qasm2 import dump
from qiskit.quantum_info import Operator, random_unitary
from qiskit.circuit.library import DiagonalGate

# ======================================================
# Configuration & Metrics [cite: 7, 76, 80, 81]
# ======================================================
BASIS_GATES = ["h", "t", "tdg", "cx"]
theta = np.pi / 7

def operator_norm_distance(U_exact, U_approx):
    """Metric: min_phi || U - exp(i phi) U_approx ||_op [cite: 77]"""
    overlap = np.trace(U_exact.conj().T @ U_approx)
    phase = overlap / abs(overlap) if abs(overlap) > 1e-14 else 1.0
    diff = U_exact - phase * U_approx
    return la.svdvals(diff)[0]

def compile_and_evaluate(name, qc):
    """Transpile to Clifford+T and save to OpenQASM """
    qc_opt = transpile(qc, basis_gates=BASIS_GATES, optimization_level=3)
    dump(qc_opt, f"{name}.qasm")

    U_exact = Operator(qc).data
    U_approx = Operator(qc_opt).data
    dist = operator_norm_distance(U_exact, U_approx)
    tcnt = qc_opt.count_ops().get("t", 0) + qc_opt.count_ops().get("tdg", 0)

    print(f"{name:18s} | T-count = {tcnt:6d} | Dist = {dist:.3e}")

# ======================================================
# 1-11 Challenge Implementation [cite: 17]
# ======================================================

# 1. Controlled-Y Gate (Sanity Check) [cite: 18, 24]
qc1 = QuantumCircuit(2); qc1.cy(0, 1)
compile_and_evaluate("01_controlled_y", qc1)

# 2. Controlled-Ry(pi/7) [cite: 25]
qc2 = QuantumCircuit(2); qc2.cry(theta, 0, 1)
compile_and_evaluate("02_cry_pi_7", qc2)

# 3. exp(i pi/7 Z⊗Z) [cite: 28]
qc3 = QuantumCircuit(2); qc3.rzz(-2 * theta, 0, 1)
compile_and_evaluate("03_exp_zz", qc3)

# 4. exp(i pi/7 (XX + YY)) [cite: 30]
qc4 = QuantumCircuit(2)
# Basis change: HXH=Z and SXSdg=Y [cite: 86, 88]
qc4.h([0, 1]); qc4.rzz(-2 * theta, 0, 1); qc4.h([0, 1])
qc4.sdg([0, 1]); qc4.h([0, 1]); qc4.rzz(-2 * theta, 0, 1); qc4.h([0, 1]); qc4.s([0, 1])
compile_and_evaluate("04_exp_h1", qc4)

# 5. exp(i pi/2 (XX + YY + ZZ)) [cite: 31]
# Hint: Resembles a SWAP gate [cite: 32]
qc5 = QuantumCircuit(2); qc5.swap(0, 1)
compile_and_evaluate("05_swap_exact", qc5)

# 6. Ising Model: exp(i pi/7 (XX + ZI + IZ)) [cite: 33]
qc6 = QuantumCircuit(2)
# Trotter step [cite: 92, 93]
qc6.rz(-2 * theta, 0); qc6.rz(-2 * theta, 1)
qc6.h([0, 1]); qc6.rzz(-2 * theta, 0, 1); qc6.h([0, 1])
compile_and_evaluate("06_ising", qc6)

# 7. State preparation (seed=42) [cite: 34, 39]
# state = [0.1061479384-0.6796414671j, -0.3622775887-0.4536131361j, 
#          0.2614190429+0.04453309691j, 0.3276449279-0.1101628411j]
# qc7 = QuantumCircuit(2); qc7.initialize(state, [0, 1])
# compile_and_evaluate("07_state_prep", qc7)

# 8. Structured Unitary 1 [cite: 40]
# Use previous results (like problem 4/6 structure)
qc8 = QuantumCircuit(2); qc8.h(0); qc8.cx(0, 1); qc8.rz(theta, 1); qc8.cx(0, 1); qc8.h(0)
compile_and_evaluate("08_structured_1", qc8)

# 9. Structured Unitary 2 (2-qubit QFT) [cite: 42, 43]
qc9 = QuantumCircuit(2); qc9.h(0); qc9.cp(np.pi/2, 1, 0); qc9.h(1); qc9.swap(0, 1)
compile_and_evaluate("09_qft", qc9)

# 10. Random Unitary (seed=42) [cite: 46, 54]
u_rand = random_unitary(4, seed=42)
qc10 = QuantumCircuit(2); qc10.unitary(u_rand, [0, 1])
compile_and_evaluate("10_random", qc10)

# 11. 4-qubit Diagonal Unitary [cite: 55]
phi = [0, np.pi, 1.25*np.pi, 1.75*np.pi, 1.25*np.pi, 1.75*np.pi, 1.5*np.pi, 1.5*np.pi,
       1.25*np.pi, 1.75*np.pi, 1.5*np.pi, 1.5*np.pi, 1.5*np.pi, 1.5*np.pi, 1.75*np.pi, 1.25*np.pi]
qc11 = QuantumCircuit(4); qc11.append(DiagonalGate(np.exp(1j * np.array(phi))), [0, 1, 2, 3])
compile_and_evaluate("11_diagonal", qc11)